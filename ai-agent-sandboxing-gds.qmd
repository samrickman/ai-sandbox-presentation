...
::: {.emphasis}
I would rather put different coding agents behind one external boundary than rely on each agent's own controls.
:::

::: {.caption}
For these tests I used <code>--clone</code> mode, so the agent could not change my host working tree.
:::
...
Docker Sandboxes has a **Balanced** network policy as its recommended starting point.

My default allowlist included:

- package registries such as npm, PyPI and crates.io
- GitHub, GitLab and Bitbucket
- broad AWS and Google Cloud infrastructure rules
- other developer services

::: {.emphasis}
Useful development infrastructure can also be an exfiltration path.
:::
...
Can the agent read or damage things outside its workspace?
...
Can information it can read leave the workspace?
...
What actions can it take in external systems?
